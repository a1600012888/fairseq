...
bert_model_config = {
    ...
    "bert_model_checkpoint": "{}".format(args.c),
...
def task(name, n_sentences, metric, criterion, regression, n_classes, data_path):
    return locals()
...
cola = (
    ...
    params(["100 200 300 400 500", "600 700 800 900 1000"], ["10"], ["16 32"], ["0.00001 0.00002 0.00003 0.00004", "0.00005 0.00006 0.00007 0.00008"], ["0.1"])
) # 60s / epoch, 3h / search
mrpc = (
    task("MRPC", 3668, "acc_f1", "cross_entropy_classify_binary", "", 2, "MRPC"), # change original num_class 1 to 2
    params(["100 200 300 400 500", "600 700 800 900 1000"], ["10"], ["16 32"], ["0.00001 0.00002 0.00003 0.00004", "0.00005 0.00006 0.00007 0.00008"], ["0.1"])
) # 50s / epoch, 3h / search
sts = (
    task("STS-B", 5749, "pearson_spearman", "mean_squared_error", "--regression-target", 1, "STS-B"),
    params(["100 200 300 400 500", "600 700 800 900 1000"], ["10"], ["16 32"], ["0.00003 0.00004 0.00005 0.00006", "0.00007 0.00008 0.00009 0.0001"], ["0.1"])
) # 50s / epoch, 4h / search
rte = (
...
mnli = (
    task("MNLI", 392702, "accuracy", "cross_entropy_classify", "", 3, "MNLI"),
    params(["100 200 300"], ["10"], ["16 32"], ["0.00001", "0.00002", "0.00003", "0.00004"], ["0.1"])
) # 5000s / epoch, bs 32 oom
mnlimm = (
    task("MNLI-mm", 392702, "accuracy", "cross_entropy_classify", "", 3, "MNLI-mm"),
    params(["100 200 300"], ["10"], ["16 32"], ["0.00001", "0.00002", "0.00003", "0.00004"], ["0.1"])
) # 5000s / epoch, bs 32 oom
qnli = (
    task("QNLI", 108436, "accuracy", "cross_entropy_classify", "", 2, "QNLI"),
    params(["100 200 300"], ["10"], ["16 32"], ["0.00001", "0.00002", "0.00003", "0.00004"], ["0.1"])
) # 1600s / epoch, bs 32 oom
qqp = (
    task("QQP", 363849, "acc_f1", "cross_entropy_classify_binary", "", 2, "QQP"),
    params(["100 200 300"], ["10"], ["16 32"], ["0.00001", "0.00002", "0.00003", "0.00004"], ["0.1"])
...
task_list = [rte, cola, mrpc, sst, qnli]#rte, cola, mrpc, sst, qnli, qqp, sts]
...
OUTPUT_PATH=${PROCEDURE_FOLDER}/GLUE/${CHECKPOINT_FILE}/${PROBLEM}/${N_EPOCH}-${BATCH_SZ}-${LR}-${WEIGHT_DECAY}-$SEED-0.006
mkdir -p $OUTPUT_PATH
...
python3 train.py $DATA_PATH/${PROBLEM}-bin \
       ...
       --tensorboard-logdir $TENSORBOARD_LOG/${PROBLEM}/${N_EPOCH}-${BATCH_SZ}-${LR}-${WEIGHT_DECAY}-$SEED-0.006 --valid-subset valid\
