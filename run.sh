...
python3 train.py --fp16 ${DATA_DIR} --ddp-backend=no_c10d \
    ...
    --arch roberta_small-stoch-attn --attn-expand 4 --attn-name random  --sample-break-mode complete --tokens-per-sample ${TOKENS_PER_SAMPLE} \
    ...
    --save-interval-updates 50000 --keep-interval-updates 6 --no-epoch-checkpoints --skip-invalid-size-inputs-valid-test --save-dir ~/exps/fairseq/${EXEC_ID}
