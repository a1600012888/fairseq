...
class MaskedAdLmLoss(FairseqCriterion):
    ...
    def __init__(self, args, task):
        ...
        self.margins = self.register_buffer(torch.zeros(self.vocab.count, 1))
    ...
    def add_args(parser):
        ...
        parser.add_argument('--margin_lambda', default=100.0, type=float, metavar='D',
                            help='weight for the adaptive margin loss')
        parser.add_argument('--margin_lr', default=0.01, type=float, metavar='D',
                            help='weight for the adaptive margin loss')
    ...
    def forward(self, model, sample, reduce=True):
        ...
        one_hot = F.one_hot(targets, self.vocab.count)

        m = F.embedding(targets, self.margins).squeeze(dim=-1)
        ...
        margin_norm = torch.sum(margin)

        total_loss = loss - margin_norm * self.margin_lambda
        logging_output = {
    ...
    def reduce_metrics(logging_outputs) -> None:
        ...
        metrics.log_scalar('margin_norm', margin_n / sample_size / math.log(2), sample_size, round=3)
