...
@register_model('roberta_deepsup')
class RobertaModel(FairseqLanguageModel):
    ...
    def forward(self, src_tokens, features_only=False, return_all_hiddens=False,
                classification_head_name=None, deepsup=False, **kwargs):
        if classification_head_name is not None:
            features_only = True

        x, extra = self.decoder(src_tokens, features_only, return_all_hiddens,
                                deepsup=deepsup, **kwargs)
...
class RobertaEncoder(FairseqDecoder):
    ...
    def forward(self, src_tokens, features_only=False, return_all_hiddens=False, masked_tokens=None,
        ...
        """
        return_all_hiddens = return_all_hiddens or deepsup
        ...
        #IPython.embed()
        if deepsup:
            mid_feature = extra['inner_states'][self.mid_layer_idx].transpose(0, 1)

            extra_predict = self.mid_lm_head(mid_feature, masked_tokens=masked_tokens)
            extra['mid_pred'] = extra_predict
        return x, extra
...
@register_model_architecture('roberta_deepsup', 'roberta_base_deepsup')
def roberta_base_architecture(args):
    base_architecture(args)


@register_model_architecture('roberta_deepsup', 'roberta_large_deepsup')
...
@register_model_architecture('roberta_deepsup', 'xlm_deepsup')
