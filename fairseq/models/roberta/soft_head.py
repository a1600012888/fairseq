...
@with_incremental_state
class LeanedScalingKQOAttention(nn.Module):
    """Multi-headed attention.
    ...
    def __init__(
        ...
        self.scale_k = Parameter(torch.zeros((1, 1, embed_dim)))
        self.scale_q = Parameter(torch.zeros((1, 1, embed_dim)))
        self.scale_o = Parameter(torch.zeros((1, 1, embed_dim)))
    ...
    def forward(
        ...
        scale_q = F.sigmoid(self.scale_q) * 2.0
        scale_k = F.sigmoid(self.scale_k) * 2.0
        scale_o = F.sigmoid(self.scale_o) * 2.0
...
class TransformerSentenceEncoderLayerWithSoftAttn(nn.Module):
    """
...
def init_bert_params(module):
    ...
    if isinstance(module, FirstSEKQOAttention) or isinstance(module, LeanedScalingKQOAttention):
        module.q_proj.weight.data.normal_(mean=0.0, std=0.02)
        module.k_proj.weight.data.normal_(mean=0.0, std=0.02)
        module.v_proj.weight.data.normal_(mean=0.0, std=0.02)


class TransformerSentenceEncoderWithSoftAttn(nn.Module):
    """
    ...
    def __init__(
        ...
        self.layers = nn.ModuleList(
            [
                TransformerSentenceEncoderLayerWithSoftAttn(
...
soft_attns=[FirstSEKQOAttention, LeanedScalingKQOAttention]