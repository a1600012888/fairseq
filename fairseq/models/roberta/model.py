...
@register_model('roberta_leaner')
class RobertaMaskLeaner(FairseqLanguageModel):
    ...
    def __init__(self, args, encoder, masker):
        super().__init__(encoder)
        self.args = args

        self.masker = masker
        # We follow BERT's random weight initialization
        self.apply(init_bert_params) # also init the masker?
...
@register_model_architecture('roberta_leaner', 'roberta_leaner')
def base_architecture(args):
    args.encoder_layers = getattr(args, 'encoder_layers', 12)
    args.encoder_embed_dim = getattr(args, 'encoder_embed_dim', 768)
    args.encoder_ffn_embed_dim = getattr(args, 'encoder_ffn_embed_dim', 3072)
