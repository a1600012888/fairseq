...
@register_model('electra_masker')
class ElectraMasker(FairseqLanguageModel):
    ...
    def __init__(self, args, gen_encoder, disc_encoder, masker):
    ...
    def build_model(cls, args, task):
        ...
        masker = MaskerEncoder(args, task.source_dictionary, disc_encoder.sentence_encoder.embed_tokens, ratio=args.masker_size_divider)

        if args.task == 'electra':
            gen_encoder = GeneratorEncoder(args, task.source_dictionary, disc_encoder.sentence_encoder.embed_tokens)
        else:
            gen_encoder = None
        return cls(args, gen_encoder, disc_encoder, masker)
...
class MaskerEncoder(FairseqDecoder):
    """RoBERTa encoder.
...
class GeneratorLMHead(nn.Module):
    ...
    def forward(self, features, masked_tokens=None, **kwargs):
        ...
        #print(features.shape, masked_tokens.shape)
        if masked_tokens is not None:
...
@register_model_architecture('electra_masker', 'electra_masker')
def base_architecture(args):
    ...
    args.masker_size_divider = getattr(args, 'masker_size_divider', 3)
...
@register_model_architecture('electra_masker', 'electra_base_masker')
def Electra_base_architecture(args):
    base_architecture(args)


@register_model_architecture('electra_masker', 'electra_small_masker')
...
@register_model_architecture('electra_masker', 'electra_large_masker')
def Electra_small_architecture(args):
